...
```bash
localitas-core app-store add --name news --compose ./docker-compose.yml --port 9204
localitas-core app-store start news
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-news:latest`. To publish a new version:
